...
<div style="width:100%; background: #6699cc;color:white;" >
  <!-- menu -->
  <span style="position:absolute; top:0px;right:0px;margin:10px;">
    <a href="logout.php"><?=_('logout'); ?></a>&nbsp;|&nbsp;
    <a href="javascript:Request('scontent','faq.php');"><?=_('faq'); ?></a>&nbsp;|&nbsp;
  ...
  <div style="overflow:none; float:left; ">
    <form onsubmit="javascript:such(); return false" action="#">
      <div style="padding: 20px; float:left; ">
        <span style="font-size: 20pt;color:white;"><?=_('Quassel Backlog Search'); ?></span>
        <br />
        <br />
      ...
      <div id="advanced" style="margin: 5px; padding: 7px; padding-left: 15px; float:left;display:none; font-size:7pt; border-left:1px solid white;">
        <span onclick="hide_a_search();" style="margin-left:-10px;" title="<?=_('Close'); ?>">[ <?=_('Advanced search'); ?> ]</span>
        <br />
        <?=_('Search in a timeperiod:'); ?><br>
        <input 
          title="<?=_('Excepts any english timeformat'); ?>" 
          name="time_start" 
          id="time_start" 
          size="20;" 
          value="Starttime" 
          onBlur="if(this.value=='') this.value='Starttime';" 
          onFocus="if(this.value=='Starttime') this.value='';">
        <input 
          title="<?=_('Excepts any english timeformat'); ?>" 
          name="time_end" 
          id="time_end" 
          size="20;" 
          value="Endtime" 
          onBlur="if(this.value=='') this.value='Endtime';" 
          onFocus="if(this.value=='Endtime') this.value='';">
        <br />
        <input style="display:none;" name="sorting" type="checkbox" id="sorting" title="<?=_('Switch between DESC and ASC-Sorting; Default: DESC'); ?>">
        <span onclick="multiple();" style="margin-left:0px;" title="<?=_('Switch between multi-search and single-search; Multisearch allows to search more than one Chat at once.'); ?>">
          <?=_('Switch multiple'); ?>
        </span>
        <div style="clear:both;<? if($backend == "sqlite"){ echo ' display:none;"'; } ?>">
          <label for="regexid"><?=_('Regex:'); ?></label>
          <input type="checkbox" name="regexid" id="regexid" title="<?=_('search with regular expression'); ?>" style="margin-right:15px;">
          <label for="types"><?=_('Only messages:'); ?></label>
          <input type="checkbox" name="types" id="types" title="<?=_('search only messages - if _not_ checked joins,quits,... will be shown and searched'); ?>" checked="checked">
        </div>
      </div>
    </form>
  </div>
    ...
    <div style="clear:both;width:100%;">
      ...
      <div id="scontent" style="overflow:auto;padding: 20px; text-align:left; vertical-align:top;">
        <font style="font-size:8pt;">
          <center><?=_('Waiting for search ...'); ?></center>
        </font>
      </div>
    </div>